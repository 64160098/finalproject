...
<x-appadmin-layout>
                    ...
                    <div class="container mt-2">
                        <div class="space-y-6">
                            ...
                            <table width="100%" border="1" cellpadding="5" cellspacing="0" class="text-gray-900 dark:text-gray-100">
                                <tbody>
                                    <tr>
                                        <td width="8%" align="left" valign="middle"><strong>วันที่</strong></td>
                                    ...
                                    @foreach ($ordernows as $ordernow)
                                    <tr>
                                        <td width="8%" align="left" valign="middle">{{ $ordernow->order_date }}</td>
                                        <td width="10%" align="left" valign="middle">{{ $ordernow->id }}</td>
                                        <td width="8%" align="left" valign="middle">{{ $ordernow->supplier_id  }}</td>
                                        <td width="10%" align="left" valign="middle">{{ $ordernow->employee->employee_firstname }} {{ $ordernow->employee->employee_lastname }}</td>
                                        <td width="4%" align="center" valign="middle">
                                            <a href="{{ route('ordernow.detailorder', $ordernow->id) }}">
                                        ...
                                        <td width="1%" align="center" valign="middle">
                                            <a href="{{ route('ordernow.editstep1', $ordernow->id) }}">
                                        ...
                                        <td width="1%" align="center" valign="middle">
                                            <a href="#" class="delete-button" data-id="{{ $ordernow->id }}">
    ...
    <script>
        $('.delete-button').click(function(event){
            ...
            if (confirmDelete) {
                var ordernowId = $(this).data('id');
                
                // ส่งคำขอลบไปยังเซิร์ฟเวอร์
                $.ajax({
                    url: "{{ route('ordernow.destroy', ':id') }}".replace(':id', ordernowId),
                    type: 'DELETE',
                    data: {
                        "_token": "{{ csrf_token() }}"
                    },
