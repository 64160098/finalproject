                                    ...
                                    <div class="p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg flex items-center gap-4">
                                        <div>
                                            ...
                                            <x-text-input wire:model="id" id="id"
                                                name="id" value="{{ $productunit->id }}" type="text" class="mt-1 block w-full"
                                    ...
                                    <div class="p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg flex items-center gap-4">
                                        <div>
                                            ...
                                            <x-text-input wire:model="unit" id="unit"
                                                name="unit" value="{{ $productunit->unit }}" type="text" class="mt-1 block w-full"
                                                required autofocus autocomplete="unit" placeholder="หน่วยนับ"/>
                                                <div class="alert alert-danger" id="unit_error"></div>
                                            @error('unit')
                                                <div class="aleart alert-success">{{ $message }}</div>
                                            @enderror
                                        </div>
    ...
    <script>
        $(document).ready(function(){
            $('#update-button').click(function(event){
                ...
                $.ajax({
                    url: '{{ route('productunit.update', $productunit->id) }}', // URL ของการส่งข้อมูล
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(response){
                        ...
                        alert('แก้ไขข้อมูลหน่วยนับเรียบร้อยแล้ว');
