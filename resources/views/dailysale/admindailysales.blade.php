                        ...
                        <div class="space-y-6">
                            ...
                            <div class="flex justify-between">
                                <p class="bread"><span><a href="{{ route('dailysale.adminmonthlysales') }}"
                                            class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">รายเดือน</a></span>
                                    / <span>รายวัน</span></p>
                                <form method="GET" action="{{ route('dailysale.admindailysales') }}">
    ...
    <script>
        $('.delete-button').click(function(event){
            ...
            if (confirmDelete) {
                ...
                $.ajax({
                    url: "{{ route('dailysale.admindestroy', ':id') }}".replace(':id', productId),
                    type: 'DELETE',
                    data: {
                        "_token": "{{ csrf_token() }}"
                    },
