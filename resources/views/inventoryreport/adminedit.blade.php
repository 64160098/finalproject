                    ...
                    <div class="contrainer mt-2">
                        <div class="row">
                            <div class="flex items-center gap-4">
                                <p class="bread"><span><a href="{{ route('inventoryreport.admininventoryreports') }}"
                                    ...
                                    / <span>แก้ไขรายงานสินค้าคงเหลือ</span></p>
                            </div>
                            @if (session('status'))
                            ...
                            @endif
                            <section>
                                <form id="inventoryreportForm" action="{{ route('inventoryreport.admin.update', $inventoryreport->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                <div class="space-y-6">
                                ...
                                <div id="total_inventory_value" class="font-bold text-xl mt-4">ราคารวม: {{ number_format($inventoryreport->total_inventory_value, 2) }}</div>
    ...
    <script>
        $(document).ready(function(){
            $('#update-button').click(function(event){
                ...
                var formData = new FormData($('#inventoryreportForm')[0]); // สร้าง FormData จากฟอร์ม
                ...
                $.ajax({
                    url: '{{ route('inventoryreport.admin.update', $inventoryreport->id) }}', // URL ของการส่งข้อมูล
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(response){
                        ...
                        alert('แก้ไขรายงานสินค้าคงเหลือเรียบร้อยแล้ว');
    ...
    <script>
        $(document).ready(function() {
            $('#employee_id').on('change', function() {
                ...
                if (id) {
                    $.ajax({
                        url: '/employee-details-inventoryreport/' + id,
                        type: 'GET',
                        success: function(response) {
