                        ...
                        <div class="space-y-6">
                            ...
                            <table width="100%" border="1" cellpadding="5" cellspacing="0" class="text-gray-900 dark:text-gray-100">
                                ...
                                @foreach ($productDetails as $product)
                                    <tr>
                                        <td>{{ $product['product']->product_name ?? 'ไม่พบข้อมูลสินค้า' }}</td>
                                        <td>{{ $product['reported_quantity'] }}</td>
                                        <td>{{ $product['product']->productUnit->unit ?? 'N/A' }}</td>
                                        <td>{{ $product['product']->price ?? 'N/A' }} บาท</td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
                            ...
                            <hr class="my-4 border-gray-300 dark:border-gray-700"> 
                            <h3 class="text-l mb-4"><strong>ตรวจสอบความถูกต้องของจำนวนสินค้า</strong></h3>      
                            <table width="100%" border="1" cellpadding="5" cellspacing="0" class="text-gray-900 dark:text-gray-100">
                                <tbody>
                                    <tr>
                                        <td width="10%" align="left" valign="middle"><strong>ชื่อสินค้า</strong></td>
                                        <td width="5%" align="left" valign="middle"><strong>จำนวนที่รายงาน</strong></td>
                                        <td width="5%" align="left" valign="middle"><strong>จำนวนในคลังจริง</strong></td>
                                        <td width="5%" align="left" valign="middle"><strong>หน่วยนับ</strong></td>
                                        <td width="5%" align="left" valign="middle"><strong>ราคา</strong></td>
                                        <td width="5%" align="left" valign="middle"><strong>สถานะ</strong></td>
                                    </tr>
                                    @foreach ($productDetails as $product)
                                        <tr>
                                            <td>{{ $product['product']->product_name ?? 'ไม่พบข้อมูลสินค้า' }}</td>
                                            ...
                                            <td>{{ $product['product']->price ?? 'N/A' }} บาท</td>
                                            <td>
                                                @if ($product['status'] === 'ตรงกัน')
                                                    <span style="color: green;">{{ $product['status'] }}</span>
                                                @elseif ($product['status'] === 'ไม่ตรงกัน')
                                                    <span style="color: red;">{{ $product['status'] }}</span>
                                                @else
                                                    <span style="color: orange;">{{ $product['status'] }}</span>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
