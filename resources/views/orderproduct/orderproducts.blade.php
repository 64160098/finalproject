...
$totalCost = 0;
    ...
    $totalQuantity += $orderproduct->quantity_products_order;
    $totalCost += $orderproduct->quantity_products_order * $orderproduct->cost_unit;
}
?>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="container mt-2">
                        <div class="space-y-6">
                            <div>
                                <a href="{{ route('orderproduct.create') }}" class=""><x-primary-button>เลือกข้อมูลสินค้า</x-primary-button></a>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <form class="product-form" action="{{ route('orderhistory.store') }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                <input type="hidden" name="code" value="{{ $orderproduct->code }}">
                                                <input type="hidden" name="product_name" value="{{ $orderproduct->product_name }}">
                                                <input type="hidden" name="quantity_products_order" value="{{ $orderproduct->quantity_products_order }}">
                                                <input type="hidden" name="unit" value="{{ $orderproduct->unit }}">
                                                <input type="hidden" name="cost_unit" value="{{ $orderproduct->cost_unit }}">
                                                <input type="hidden" name="total" value="{{ $orderproduct->total }}">

                                            <td width="10%" align="left" valign="middle">
                                                <input type="checkbox" name="selected_products[]" value="{{ $orderproduct->id }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $orderproduct->code }}
                                                <input type="hidden" name="code" value="{{ $orderproduct->code }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $orderproduct->product_name }}
                                                <input type="hidden" name="product_name" value="{{ $orderproduct->product_name }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $orderproduct->quantity_products_order }}
                                                <input type="hidden" name="quantity_products_order" value="{{ $orderproduct->quantity_products_order }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $orderproduct->unit }}
                                                <input type="hidden" name="unit" value="{{ $orderproduct->unit }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $orderproduct->cost_unit }}
                                                <input type="hidden" name="cost_unit" value="{{ $orderproduct->cost_unit }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $orderproduct->total }}
                                                <input type="hidden" name="total" value="{{ $orderproduct->total }}">
                                            </td>
                                            </form>  
                                            <td width="10%" align="center" valign="middle">
                                                <a href="{{ route('orderproduct.edit', $orderproduct->id) }}">
                                            ...
                                            <td width="10%" align="center" valign="middle"> 
                                                <a href="#" class="delete-button" data-id="{{ $orderproduct->id }}">
                                                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
...
<script>
    $(document).ready(function(){
        $('#confirm-button').click(function(event){
            ...
            $.ajax({
                url: "{{ route('orderlist.store') }}", // URL ของการส่งข้อมูล
                type: 'POST',
                ...
                success: function(response){
                    ...
                    if (response.success) {
                        ...
                        $.ajax({
                            url: "{{ route('orderproduct.deleteAll') }}",
                            type: 'DELETE',
                            headers: {
    ...
    <script>
        $('.delete-button').click(function(event){
            ...
            if (confirmDelete) {
                ...
                $.ajax({
                    url: "{{ route('orderproduct.destroy', ':id') }}".replace(':id', productId),
                    type: 'DELETE',
                    data: {
                        "_token": "{{ csrf_token() }}"
                    },
                    success: function(response){
                        ...
                        window.location.href = "{{ route('orderproduct.orderproducts') }}";
                    },
                    error: function(xhr){
