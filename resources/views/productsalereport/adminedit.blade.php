                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="contrainer mt-2">
                        <div class="row">
                            ...
                            @endif
                            <section>
                                <form action="{{ route('productsalehistory.update', $productsalehistory->id) }}" method="POST"
                                    ...
                                    <div class="space-y-6">
                                        ...
                                        <div class="p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg flex items-center gap-4">
                                            <div>
                                                ...
                                                {{ $productsalehistory->product_name }}
                                                @error('product_name')
                                                    <div class="aleart alert-success">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg flex items-center gap-4">
                                            <div>
                                                <x-input-label for="quantity_products_sale" :value="__('จำนวนที่ขายได้')" />
                                                <x-text-input wire:model="quantity_products_sale" id="quantity_products_sale"
                                                    name="quantity_products_sale" value="{{ $productsalehistory->quantity_products_sale }}" type="number" class="mt-1 block w-full"
                                                    required autofocus autocomplete="quantity_products_sale" placeholder="จำนวนสินค้าที่สั่ง"/>
                                                @error('quantity_products_sale')
                                                    <div class="aleart alert-success">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        </div>
