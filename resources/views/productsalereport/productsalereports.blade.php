...
$totalCost = 0;
    ...
    foreach ($productsalereports as $productsalereport) {
    $totalQuantity += $productsalereport->quantity_products_sale;
    $totalCost += $productsalereport->quantity_products_sale * $productsalereport->cost_unit;
}
?>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="container mt-2">
                        <div class="space-y-6">
                            <div>
                                <a href="{{ route('productsalereport.create') }}" class=""><x-primary-button>เลือกข้อมูลสินค้า</x-primary-button></a>
                            </div>
                                ...
                                <table id="producttable" width="100%" border="1" cellpadding="5" cellspacing="0">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td width="10%" align="left" valign="middle"><strong>จำนวนที่ขาย</strong></td>
                                        ...
                                        @foreach ($productsalereports as $productsalereport)
                                        <tr>
                                            <form class="product-form" action="{{ route('productsalehistory.store') }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                <input type="hidden" name="code" value="{{ $productsalereport->code }}">
                                                <input type="hidden" name="product_name" value="{{ $productsalereport->product_name }}">
                                                <input type="hidden" name="quantity_products_sale" value="{{ $productsalereport->quantity_products_sale }}">
                                                <input type="hidden" name="unit" value="{{ $productsalereport->unit }}">
                                                <input type="hidden" name="cost_unit" value="{{ $productsalereport->cost_unit }}">
                                                <input type="hidden" name="total" value="{{ $productsalereport->total }}">

                                            <td width="10%" align="left" valign="middle">
                                                <input type="checkbox" name="selected_products[]" value="{{ $productsalereport->id }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $productsalereport->code }}
                                                <input type="hidden" name="code" value="{{ $productsalereport->code }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $productsalereport->product_name }}
                                                <input type="hidden" name="product_name" value="{{ $productsalereport->product_name }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $productsalereport->quantity_products_sale }}
                                                <input type="hidden" name="quantity_products_sale" value="{{ $productsalereport->quantity_products_sale }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $productsalereport->unit }}
                                                <input type="hidden" name="unit" value="{{ $productsalereport->unit }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $productsalereport->cost_unit }}
                                                <input type="hidden" name="cost_unit" value="{{ $productsalereport->cost_unit }}">
                                            </td>
                                            <td width="10%" align="left" valign="middle">{{ $productsalereport->total }}
                                                <input type="hidden" name="total" value="{{ $productsalereport->total }}">
                                            </td>
                                            </form>  
                                            <td width="10%" align="center" valign="middle">
                                                <a href="{{ route('productsalereport.edit', $productsalereport->id) }}">
                                            ...
                                            <td width="10%" align="center" valign="middle"> 
                                                <a href="#" class="delete-button" data-id="{{ $productsalereport->id }}">
                                                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <div id="submit-btn">
                            <x-primary-button id="confirm-button"> ยืนยันการส่งออเดอร์ </x-primary-button>
    ...
    <script>
        $(document).ready(function(){
            $('#confirm-button').click(function(event){
                ...
                $.ajax({
                    url: "{{ route('productsalehistory.store') }}", // URL ของการส่งข้อมูล
                    type: 'POST',
                    ...
                    success: function(response){
                        ...
                        if (response.success) {
                            ...
                            $.ajax({
                                url: "{{ route('productsalereport.deleteAll') }}",
                                type: 'DELETE',
                                headers: {
