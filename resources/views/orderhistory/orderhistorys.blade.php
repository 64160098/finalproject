...
<x-appadmin-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-end">
                <small class="text-gray-900 dark:text-gray-100">**หากต้องการค้นหาวันที่รายงาน <br>**โปรดระบุวันที่ในรูปแบบ YYYY-MM-DD เช่น 2024-01-01</small>
            </div>   
                            ...
                            <table width="100%" border="1" cellpadding="5" cellspacing="0" class="text-gray-900 dark:text-gray-100">
                                <tbody>
                                    <tr>
                                        ...
                                        <td width="10%" align="left" valign="middle"><strong>วันที่รับสินค้า</strong></td>
                                        <td width="8%" align="left" valign="middle"><strong>เลขที่บริษัท</strong></td>
                                        <td width="8%" align="left" valign="middle"><strong>รหัสผู้สั่งซื้อ</strong></td>
                                        <td width="8%" align="left" valign="middle"><strong>สถานะ</strong></td>
                                        <td colspan="3" width="15%" align="center" valign="middle"><strong>Action</strong></td>
                                    </tr>
                            
                                    @foreach ($orderhistorys as $orderhistory)
                                    <tr>
                                        <td width="10%" align="left" valign="middle">{{ $orderhistory->order_id  }}</td>
                                        <td width="10%" align="left" valign="middle">{{ $orderhistory->received_date->format('Y-m-d') }}</td> 
                                        <td width="8%" align="left" valign="middle">{{ $orderhistory->supplier_id  }}</td>                              
                                        <td width="8%" align="left" valign="middle">{{ $orderhistory->employee_id   }}</td>  
                                        <td width="8%" align="left" valign="middle">{{ $statuses[$orderhistory->order_id] ?? 'ข้อมูลไม่พบ' }}</td>  
                                        <td width="4%" align="center" valign="middle">
                                            <a href="{{ route('orderhistory.detailorderhistory', ['order_id' => $orderhistory->order_id]) }}">
                                        ...
                                        <td width="1%" align="center" valign="middle">
                                            <a href="{{ route('orderhistory.edit', $orderhistory->order_id) }}">
                                        ...
                                        <td width="1%" align="center" valign="middle"> 
                                            <a href="#" class="delete-button" data-id="{{ $orderhistory->order_id }}">
            ...
            if (confirmDelete) {
                ...
                $.ajax({
                    ...
                    error: function(xhr){
                        ...
                        // ตรวจสอบการตอบสนองของ server เพื่อดึงข้อความข้อผิดพลาด
                        var errorMessage = 'เกิดข้อผิดพลาดในการลบข้อมูล'; // ข้อความเริ่มต้น

                        if (xhr.responseJSON) {
                            if (xhr.responseJSON.message) {
                                errorMessage = xhr.responseJSON.message; // ข้อความข้อผิดพลาดที่ส่งมาจาก server
                            } else if (xhr.responseJSON.error) {
                                errorMessage = xhr.responseJSON.error; // ข้อความข้อผิดพลาดที่ส่งมาจาก server
                            }
                        }
