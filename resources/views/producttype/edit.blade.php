                        ...
                        <div class="row">
                            <div class="flex items-center gap-4">
                                <p class="bread"><span><a href="{{ route('producttype.producttypes') }}"
                                    ...
                                    / <span>แก้ไขข้อมูลประเภทสินค้า</span></p>
                            </div>
                            @if (session('status'))
                                    ...
                                    <div class="p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg flex items-center gap-4">
                                        <div>
                                            ...
                                            <x-text-input wire:model="id" id="id"
                                                name="id" value="{{ $producttype->id }}" type="text" class="mt-1 block w-full"
                                    ...
                                    <div class="p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg flex items-center gap-4">
                                        <div>
                                            ...
                                            <x-text-input wire:model="product_type" id="product_type"
                                                name="product_type" value="{{ $producttype->product_type }}" type="text" class="mt-1 block w-full" required
                                                autofocus autocomplete="product_type" placeholder="ประเภทสินค้า"/>
                                                <div class="alert alert-danger" id="product_type_error"></div>
                                            @error('product_type')
                                                <div class="aleart alert-success">{{ $message }}</div>
                                            @enderror
                                        </div>
    ...
    <script>
        $(document).ready(function(){
            $('#update-button').click(function(event){
                ...
                $.ajax({
                    url: '{{ route('producttype.update', $producttype->id) }}', // URL ของการส่งข้อมูล
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(response){
                        ...
                        alert('แก้ไขข้อมูลประเภทสินค้าเรียบร้อยแล้ว');
