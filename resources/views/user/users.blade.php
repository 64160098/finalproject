                    ...
                    <div class="container mt-2">
                        <div class="space-y-6">
                            <div class="flex justify-between">
                                <a href="{{ route('user.create') }}" class=""><x-primary-button>เพิ่มข้อมูล</x-primary-button></a>
                                <form method="GET" action="{{ route('user.index') }}">
                                    ...
                                    <br>
                                    <small class="text-gray-500">**สถานะมี 3 ระดับ ได้แก่<br>**ระดับ 1 คือ Superadmin<br>**ระดับ 2 คือ Admin<br>**ระดับ 3 คือ User</small>
                                </form>
                            </div>
                            ...
                            <table id="suppliertable" width="100%" border="1" cellpadding="5" cellspacing="0">
                                <tbody><tr>
                                    <td width="10%" align="left" valign="middle"><strong>ชื่อ</strong></td>
                                ...
                                @foreach ($users as $user)
                                 <tr>
                                    <td width="10%" align="left" valign="middle">{{ $user->name }}</td>
                                    <td width="10%" align="left" valign="middle">{{ $user->email }}</td>
                                    <td width="10%" align="left" valign="middle">{{ $user->role }}</td>
                                    ...
                                    <td width="10%" align="center" valign="middle"> 
                                        <a href="#" class="delete-button" data-id="{{ $user->id }}">
                                            <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        $('.delete-button').click(function(event){
            ...
            if (confirmDelete) {
                ...
                $.ajax({
                    url: "{{ route('user.destroy', ':id') }}".replace(':id', productId),
                    type: 'DELETE',
                    data: {
                        "_token": "{{ csrf_token() }}"
                    },
